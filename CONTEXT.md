...
| Fresh installs bootstrap a `localhost` self-monitoring host (connection=local) when the hosts table is empty after yaml seeding; it is DB-created (UI-like path), deletion sticks, and defining `localhost` in yaml later adopts it via the normal upsert | Zero-config self-monitoring on day one without resurrecting hosts operators removed | A user-defined `localhost` in yaml silently takes over connectivity from the bootstrapped row |
