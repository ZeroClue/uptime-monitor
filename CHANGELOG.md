...
- **Project-scoped alert rules + notification channels** — both tables gain nullable `project_id` (nil = global); the alert engine applies a rule only to hosts in its project and delivers notifications through matching channels only (global channels receive all alerts; unassigned-host alerts go to global channels). `/api/alert-rules` and `/api/notification-channels` list/create within the active project scope, with by-ID access isolated per project; `/alerts/config` follows the nav switcher. Also fixes rules/channels API field mapping and a long-standing syntax error that left the config page's tables blank.
- **Per-host timeout knobs** — `ssh_timeout` (connection phase, default 10s) and `collector_timeout` (whole-collect budget enforced by the scheduler, default 30s) join the existing per-command `timeout` (default raised 10s → 30s). Set in hosts.yaml or the host form; stored as nullable columns so unset = defaults. Collector budget applies across retry attempts.
- **Poll retry with exponential backoff** — failed polls retry `min(base*2^n + jitter, max)` (defaults 3 attempts / 2s base / 30s max / 0.2 jitter) via the global `retry:` block or per-host overrides (`retry_max_retries`, `retry_base_delay`, `retry_max_delay` in hosts.yaml and the host form). Auth and host-key failures skip retries. Attempt count + backoff time per poll exposed via `/api/hosts/status`. Also fixes host CRUD silently dropping snake_case fields (key_path, proxy_jump, collector_preference, tags) due to missing JSON tags.
...
- **Default-project guards** — deleting the default project returns 400 (promote another first); exactly-one-default invariant enforced transactionally on create/update.
