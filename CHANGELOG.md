...
- **Project-scoped alert rules + notification channels** — both tables gain nullable `project_id` (nil = global); the alert engine applies a rule only to hosts in its project (global rules apply everywhere); `/api/alert-rules` and `/api/notification-channels` list/create within the active project scope; `/alerts/config` follows the nav switcher. Also fixes rules/channels API field mapping (missing JSON tags meant the config UI rendered blanks).
