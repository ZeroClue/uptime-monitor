...
### Added
- **API token authentication** — `Authorization: Bearer <token>` now authenticates API requests: token hashes upgraded to SHA-256 (legacy XOR hashes migrate transparently on first use), scopes enforced (`read` < `write` < `admin`; config endpoints require admin), per-token rate limiting (60/min, 429 + Retry-After), `last_used_at` updated on use, and project-scoped tokens restrict requests to their project's data. Closes #59.
